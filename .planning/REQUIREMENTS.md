...
- [ ] **VER-01**: A logging hook records every Claude Code hook event (name + relevant payload fields + timestamp + session_id) to `~/.claude/hook-events.log`, installable alongside the existing hooks without disturbing them
