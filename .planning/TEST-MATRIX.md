...
| 15 | Multi-container, sessions in parallel | 2-3 containers, same host `.claude` | independent per-session states | one state file per session_id | session_id collisions; cwd→label mapping from hook payload | 2026-07-29 / cc 2.1.220 / PARZIALE — 2 sessioni parallele (container/cwd distinti) loggate concorrenti senza corruzione sul mount 9p condiviso, session_id distinti, ogni riga porta cwd; run deliberato a 3 container pendente |
