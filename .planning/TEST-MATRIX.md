...
| 4 | ⚠ Permission prompt | Tool not in allowlist (e.g. dangerous rm) | needs_input | `Notification` | Does it fire for ALL prompt types (Bash, MCP, file writes)? | 2026-07-29 / cc 2.1.220 / DATO — 0 Notification su 615 eventi; tutte le sessioni in bypassPermissions; una conferma tool (rm, ~09:50) NON ha emesso Notification: primo indizio che alcuni flussi di conferma siano hook-silenti. Test deliberato in default permission mode NECESSARIO |
...
| 10 | `/resume` of a past session | Resume an ended session | working on next prompt | `SessionStart` (source=resume) | Old session's leftover state file; session_id continuity | 2026-07-29 / cc 2.1.220 / PARZIALE — /resume emette SessionEnd(reason=resume) sulla sessione invocante + SessionStart(source=resume) con session_id DIVERSO; il problema dello state file orfano della vecchia sessione è reale |
...
| 14 | Clean session exit | Exit Claude Code normally | session disappears | `SessionEnd` | Fires on crash too? (overlap with #9) | 2026-07-29 / cc 2.1.220 / PARZIALE — exit pulito emette SessionEnd reason=prompt_input_exit (2×); il resume-exit usa reason=resume (distinguibili); variante crash non testata |
